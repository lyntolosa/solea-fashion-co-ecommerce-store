...
@include('partials.demo-notice')
    <main class="flex min-h-screen flex-col md:flex-row">
        ...
        <section class="admin-login-panel relative flex w-full flex-col items-center justify-center bg-surface p-6 md:w-1/2 md:p-12 lg:w-2/5 lg:p-20"><div class="w-full max-w-md"><div class="mb-10"><h2 class="mb-2 font-headline text-3xl font-bold tracking-tight md:text-4xl">Admin Login</h2><p class="text-sm text-white/50 md:text-base">Access the Soléa Fashion Co. dashboard</p></div>
            ...
            <form action="{{ route('admin.login.authenticate') }}" method="POST" class="space-y-6"><div class="space-y-1.5"><label for="email" class="ml-1 text-[10px] font-semibold uppercase tracking-widest text-white/40">Email Address</label><input id="email" name="email" value="{{ old('email', 'rctolosa.admin@soleafashion.demo') }}" class="w-full rounded-xl border-0 bg-surface-container-highest px-4 py-4 text-white placeholder:text-white/20 focus:ring-2 focus:ring-primary" placeholder="admin@threadlab.studio" required type="email" autocomplete="email"></div><div class="space-y-1.5"><label for="password" class="ml-1 text-[10px] font-semibold uppercase tracking-widest text-white/40">Password</label><input id="password" name="password" value="{{ old('password', 'RCTOLOSA2026!') }}" class="w-full rounded-xl border-0 bg-surface-container-highest px-4 py-4 text-white placeholder:text-white/20 focus:ring-2 focus:ring-primary" placeholder="Enter your password" required type="password" autocomplete="current-password"></div><div class="flex items-center justify-between py-2"><label for="remember" class="flex cursor-pointer items-center gap-3 text-xs text-white/40"><input id="remember" name="remember" type="checkbox" class="h-5 w-5 rounded border-0 bg-surface-container-highest text-primary focus:ring-primary">Remember Me</label><a href="#" class="text-xs font-bold italic text-primary transition hover:text-white">Forgot Password?</a></div><button class="btn-glow mt-4 w-full rounded-xl bg-primary py-5 font-headline text-sm font-black uppercase tracking-widest text-black transition-all active:scale-[.98]" type="submit">Login</button>@csrf</form>
