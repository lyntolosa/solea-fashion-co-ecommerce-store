...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/admin-brand.css') }}?v=20260915-2">
...
<main class="min-h-screen px-6 pb-16 pt-24 lg:ml-64 lg:px-12">
    <div class="mx-auto max-w-[1400px]">
        ...
        @endif
        @if($rows)
            <section class="overflow-x-auto border border-outline-variant/20 bg-surface"><table class="w-full min-w-[900px] text-left"><thead class="border-b border-outline-variant/20 text-xs font-bold uppercase tracking-widest text-muted"><tr><th class="px-6 py-4">{{ $section === 'products' ? 'Product' : ($section === 'customers' ? 'Customer' : ($section === 'discounts' ? 'Voucher' : 'Order')) }}</th><th class="px-6 py-4">Details</th><th class="px-6 py-4">Recorded</th><th class="px-6 py-4">Value</th><th class="px-6 py-4">Status</th></tr></thead><tbody class="divide-y divide-outline-variant/20">@foreach($rows as $row)<tr class="align-top hover:bg-surface-container-high"><td class="px-6 py-5"><p class="font-bold">{{ $row['primary'] }}</p><p class="mt-1 text-xs text-muted">{{ $row['secondary'] }}</p>@if($section === 'products')<div class="mt-3 flex flex-col items-start gap-2"><a href="{{ route('admin.products.edit', $row['slug']) }}" class="inline-flex items-center gap-1 text-xs font-bold uppercase tracking-widest text-primary"><span class="material-symbols-outlined text-sm">edit</span>Edit product</a><a href="{{ route('admin.products.edit', $row['slug']) }}" class="inline-flex items-center gap-1 border border-outline-variant/30 px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-primary hover:border-primary hover:bg-primary hover:text-white"><span class="material-symbols-outlined text-sm">tune</span>Quick edit</a></div>@endif</td><td class="max-w-md px-6 py-5 text-sm text-muted">{{ $row['detail'] }}</td><td class="px-6 py-5 text-sm text-muted">{{ $row['recorded'] ?? 'Recorded' }}</td><td class="px-6 py-5 font-bold text-primary">{!! $row['value'] !!}</td><td class="px-6 py-5">@if($section === 'orders')<form action="{{ route('admin.orders.status', $row['reference']) }}" method="POST">@csrf<select name="status" onchange="this.form.submit()" class="border border-outline-variant/30 bg-surface-container-high px-3 py-2 text-xs font-bold uppercase text-primary">@foreach($orderFilters['statuses'] as $status)<option @selected($row['status'] === $status)>{{ $status }}</option>@endforeach</select></form>@else<span class="text-xs font-bold uppercase tracking-widest text-primary">{{ $row['status'] }}</span>@endif</td></tr>@endforeach</tbody></table></section>
        @else
