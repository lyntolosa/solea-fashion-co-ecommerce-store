    ...
    <form action="{{ route('admin.inventory.products.update', $product['slug']) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <section class="border border-white/10 bg-surface p-6 md:p-8">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div><label for="scheduled_at" class="mb-2 block text-xs font-bold uppercase tracking-widest text-muted">Publish date for scheduled status</label><input id="scheduled_at" name="scheduled_at" type="datetime-local" value="{{ old('scheduled_at', !empty($product['scheduled_at']) ? \Carbon\Carbon::parse($product['scheduled_at'])->format('Y-m-d\\TH:i') : '') }}" class="w-full border border-white/15 bg-black px-4 py-3 text-white focus:border-primary focus:outline-none"><p class="mt-2 text-xs text-muted">Only used when status is Scheduled.</p></div>
